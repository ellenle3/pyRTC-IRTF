...
class RealTimeView(QMainWindow):
    def __init__(self, shm_name, fps, static_vmin=None, static_vmax=None):
        ...
        framenew = np.zeros((5,2))
        framenew[:4] = frame
        framenew[4,0] = np.average(frame[:2])
        framenew[4,1] = np.average(frame[2:])
        frame = framenew
        ...
        self.im = self.axes.imshow(frame, cmap='seismic', interpolation='nearest', aspect=aspect,
